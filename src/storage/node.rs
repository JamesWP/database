...
impl NodePage {
    ...
    /// Returns the exact CBOR-encoded byte length of this page without allocating.
    ///
    /// `OverflowPage` returns 0 because `split_and_store` always chunks data so each
    /// overflow page fits by construction.
    pub fn cbor_encoded_size(&self) -> usize {
        if matches!(self, NodePage::OverflowPage(_)) {
            return 0;
        }
        ...
        ciborium::ser::into_writer(self, &mut w).unwrap();
        w.0
    }
...
mod test {
    ...
    /// Regression: long composite keys (e.g. 51 bytes) previously caused the arithmetic
    /// estimator to undercount page size, leading to a flush panic.  The exact CBOR
    /// binary that triggered the panic is stored in tests/fixtures/panic_page_cbor.bin.
    #[test]
    fn test_leaf_cbor_encoded_size_long_keys() {
        ...
        for key_len in [24usize, 51, 100, 255, 256] {
            for cell_count in [1usize, 50, 136] {
                ...
                assert_eq!(
                    page.cbor_encoded_size(),
                    real_cbor_size(&page),
                    "key_len={key_len} cell_count={cell_count}"
                );
            }
        }
        ...
        assert_eq!(
            page.cbor_encoded_size(),
            real_cbor_size(&page),
            "panic page (136 cells, 51-byte keys)"
        );
    }
