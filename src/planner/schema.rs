...
impl Table {
    ...
    /// Columns with no explicit type are treated as integer-affinity (matching SQLite).
...
mod tests {
    ...
    fn integer_pk_does_not_create_pk_index() {
        ...
        assert!(
            indexes.iter().all(|idx| idx.index_name != "_pk_t_id"),
            "INTEGER PRIMARY KEY should not create a _pk_ implicit index"
        );
    }
}
